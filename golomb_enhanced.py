...
def compute_fractal_lines(start, angle, length, depth, ruler, max_lines, max_depth):
    ...
    while stack:
        ...
        for mark in ruler[1:]:
            ...
            new_angle = angle + ((mark % 3) - 1) * \
                (np.pi / 6)  # Diverse angles
            ...
            if line_count < max_lines:
                ...
                colors[line_count] = (
                    (depth / max_depth) + (new_angle / (2 * np.pi)) + (mark / max(ruler))) % 1.0
                line_count += 1

            stack.append((new_x, new_y, new_angle +
                         np.pi / 6, new_length, depth - 1))
            stack.append((new_x, new_y, new_angle -
                         np.pi / 6, new_length, depth - 1))
...
def plot_fractal(ax_lines, colors, line_count):
    fig, ax = plt.subplots(
        figsize=(8, 8), facecolor='black')  # Dark background
    ax.set_aspect('equal')
    ax.axis('off')
